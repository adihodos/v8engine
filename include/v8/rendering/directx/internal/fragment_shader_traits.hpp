...
///
/// \brief Fragment shader function and types.
struct dx11_fragmentshader_traits {
    typedef ID3D11PixelShader                           interface_type;
    ...
    static inline void bind_shader_to_pipeline_stage(
        ID3D11DeviceContext* device_context, interface_pointer pixel_shader
        );
...
inline dx11_fragmentshader_traits::interface_pointer
    dx11_fragmentshader_traits::create_shader(
    ID3D11Device* device, ID3D10Blob* compiledBytecode
    ) {
        HRESULT ret_code;
        ...
        CHECK_D3D(
            &ret_code,
            device->CreatePixelShader(compiledBytecode->GetBufferPointer(), 
            compiledBytecode->GetBufferSize(), 
            nullptr, &p_shader));
        return p_shader;
}

inline void dx11_fragmentshader_traits::bind_shader_to_pipeline_stage(
    ID3D11DeviceContext* device_context, interface_pointer pixel_shader
    ) {
        device_context->PSSetShader(pixel_shader, nullptr, 0);
}

inline void dx11_fragmentshader_traits::bind_uniform_blocks_to_pipeline_stage(
    ID3D11DeviceContext* device_context, 
    v8_uint32_t start_slot, 
    ID3D11Buffer* const * buffers, 
    v8_uint32_t num_buffers
...
inline void dx11_fragmentshader_traits::bind_sampler_states_to_pipeline_stage(
    ID3D11DeviceContext* device_context, 
    v8_uint32_t start_slot, 
    ID3D11SamplerState* const* sampler_states, 
    v8_uint32_t num_states
    ) {
        device_context->PSSetSamplers(start_slot, num_states, sampler_states);
}

inline void dx11_fragmentshader_traits::bind_resource_views_to_pipeline_stage( 
    ID3D11DeviceContext* device_context, 
    v8_uint32_t start_slot, 
    ID3D11ShaderResourceView* const* resource_views, 
    v8_uint32_t num_rvs
    ) {
        device_context->PSSetShaderResources(start_slot, num_rvs, resource_views);
}
